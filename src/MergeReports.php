...
class MergeHTMLReportsTask extends BaseTask implements TaskInterface, MergeReportsTaskInterface {
  ...
  /**
   * This function counts all types of tests' scenarios and writes in class
   * members
   *
  ...
  private function updateSummaryTable($dstFile) {
    ...
    $dstFile->query($pathFor('scenarioSuccessValue'))
      ->item(0)->nodeValue = $this->countSuccess;
    $dstFile->query($pathFor('scenarioFailedValue'))
      ->item(0)->nodeValue = $this->countFailed;
    $dstFile->query($pathFor('scenarioSkippedValue'))
      ->item(0)->nodeValue = $this->countSkipped;
    $dstFile->query($pathFor('scenarioIncompleteValue'))
      ->item(0)->nodeValue = $this->countIncomplete;
  }
  ...
  private function updateToolbarTable($dstFile) {
    ...
    $dstFile->query($pathFor('Successful'))
      ->item(0)->nodeValue = '✔ ' . $this->countSuccess;
    $dstFile->query($pathFor('Failed'))
      ->item(0)->nodeValue = '✗ ' . $this->countFailed;
    $dstFile->query($pathFor('Skipped'))
      ->item(0)->nodeValue = 'S ' . $this->countSkipped;
    $dstFile->query($pathFor('Incomplete'))
      ->item(0)->nodeValue = 'I ' . $this->countIncomplete;
  }

  /**
   * This function updates "+" and "-" button for viewing test steps in final
   * report
   *
   * @param $dstFile \DOMDocument - destination file
   */
